...
#define bitrev8( x)	x = ((x & 0xAA) >> 1) | ((x & 0x55) << 1); \
x = ((x & 0xCC) >> 1) | ((x & 0x33) << 1); \
x = ((x & 0xF0) >> 1) | ((x & 0x0F) << 1); \
x = ((x & 0xFF) >> 1) | ((x & 0x00) << 1);
...
//fprintf(stderr,"pix %i, bpp %i\n",p,bpp);
//static 
unsigned char is[8]={ 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80 };
//unsigned char *e= &b;
//unsigned char t=e[1];
//e[1]=e[0];
//e[0]=t;
//for(i=0;i<=7;i++)	if(b & is[i]) d= d|is[7-i];
...
C4(  0,4), C4(  1,4), C4(  2,4), C4(  3,4), C4(  4,4), C4(  5,4), C4(  6,4), C4(  7,4),
C4(  8,4), C4(  9,4), C4( 10,4), C4( 11,4), C4( 12,4), C4( 13,4), C4( 14,4), C4( 15,4),
};
