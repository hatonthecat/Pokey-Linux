...
MicroSD storage is cheap in 2026, whereas RAM is of course, not super expensive for small systems under 4GB. However, Since lowering the energy consumption of the single board computer to make it solar powered, the systems need to use less than 8MB, 16MB, and 32MB whenever possible, and an initial "LiveCD" would not include any heavy RAM software (like Libre Office), since none of those programs could fit under a 32MB RAM System Requirement, even if it is installed to the disk and executes in place. Theoretically, some optimizations could take place, but routine (in-app) disk IO to a microSD ALSO consumes power, and the purpose of including the microSD into the system is purely to load the OS into RAM, after which the microSD will unmount and run unpowered, until the user wants to load a different application (and by definition of the New Model (see SD card on right panel of this: https://github.com/hatonthecat/Pokey-Linux/blob/main/Kodak%20Carousel%20with%20OS%20analogy.jpg), the Disk is only temporarily accessed at boot, but the system is more or less a RAM disk. One other difference from conventional RAM disks (except for the smallest ones like initrd), is that those live CDS (including DSL), require 128MB RAM+ to uncompress the entire distro into RAM, and have additional RAM left over to run those programs. Thus the tradeoff is not trivial when accounting for hybrid systems that depend on I/O to load the OS, but unlike those live distros, they are only loading 1/64th (in the case of a 1GB microSD Card with sixty four 16MB pre-compiled images to run one app at a time, which is then inflated/unzipped/uncompressed into the RAM as a ramdisk (persistent storage can be set to a different partition of that same uSD disk, including for Loading and saving documents, but not with routine autosave, unless one can/wants to quickly drain the battery of the laptop, since it will run on a very lightweight lithium or sodium ion capacitor: https://www.vinatech.com/upload/prod/VEL13353R8257G_data%20sheet%20%5Bstandard%5D_Ver1.4(R0,221005)_5b1f0.pdf or uploaded to the cloud if paired with a wireless card), but only apps that can fit under 16MB after accounting for the X window system/or frame buffer system and the application itself to run. In other words, the OS might require 7MB of RAM and the app is allowed to use at maximum, 9MB. Optimizations can be designed using stress tests- creating maximum entropy e.g in a lightweight word processor like Microsoft Works or AbiWord: https://web.mit.edu/outland/share/AbiSuite/AbiWord/help/en-US/info/inforequirements.html#:~:text=Here%20are%20some%20system%20requirements%20for%20AbiWord:,or%20better%20*%20At%20least%2016MB%20RAM and then creating tens or hundreds of page breaks until the application runs out of memory. So there is a way to determine what is the maximum memory the software will allow the user to use, either indirectly, or directly- by setting a hard limit as to how many pages can be created or how many characters can be included in the clipboard and file before freezing, and therefore, pre-emptively prompting the user to limit the characters and not allowing more than that limit so as to prevent it from crashing.

https://github.com/tindzk/GTK GTK 1.2.10 and Abiword can run in as little as 16MB of RAM. Task 2.1 Figure out which kernel version is required to run GTK 1.2.x.
...
9MB Test 0 320x200 & 640x480x4 (non working color & text (wrong video uploaded- VBox didn't save my latest)
--

[Xwoaf 1.3 rebuild-screen0.webm](https://github.com/user-attachments/assets/3b706900-7c95-4373-b2ba-dfb39d8136fc)

9MB Test 3  320x200x8 & 640x480x8 (Working!)
--
