...
static BOOL CALLBACK DirProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg == WM_IN_UPDATEMSG)
  {
    ...
    {
#ifdef NSIS_CONFIG_COMPONENTPAGE
      if (!x || g_inst_section[x].name_ptr<0||g_inst_section[x].default_state&0x80000000)
#endif
...
static BOOL CALLBACK SelProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg == WM_INITDIALOG)
  {
    ...
    {
      int x=0;
      for (; x < NSIS_MAX_INST_TYPES && g_inst_header->install_types_ptr[x]>=0; x ++)
      {
      ...
      for (; x < g_inst_header->num_sections; x ++)
      {
        if (g_inst_section[x].name_ptr>=0)
        {
          ...
          if (!x || (g_inst_section[x].default_state & (0x80000000|(1<<m_whichcfg)))
              || g_inst_header->install_types_ptr[0]<0)
  ...
  if (uMsg == WM_COMMAND)
  {
    ...
    if (id == IDC_LIST1 && code==LBN_SELCHANGE)
    {
      ...
      int l=SendMessage(hwndCombo1,CB_GETCOUNT,0,0)-1;
      ...
      for (r = 0; r < l; r ++)
      {
        ...
        for (; x < g_inst_header->num_sections; x ++)
        {
          ...
          if (c)
          {
            if (x && c != '-')
            {
              if (g_inst_header->no_custom_instmode_flag==1) SendMessage(hwndList1,LB_SETSEL,(g_inst_section[x].default_state>>m_whichcfg)&1,a);
              else if (!(g_inst_section[x].default_state&(1<<r)) != !SendMessage(hwndList1,LB_GETSEL,a,0)) break;
            }
            a++;
          }
        }
  ...
  if (uMsg == WM_IN_UPDATEMSG)
  {
    ...
    {
      int c=(m_whichcfg == SendMessage(hwndCombo1,CB_GETCOUNT,0,0)-1)<<3;// SW_SHOWNA=8, SW_HIDE=0
    ...
    if (lpdis->itemID != -1)
    {
      if (lpdis->itemAction == ODA_SELECT || lpdis->itemAction == ODA_DRAWENTIRE)
      {
        ...
        else
        {
          ...
          oldbm=SelectObject(memdc,hBMcheck[!((lpdis->itemState & ODS_SELECTED)||(!lpdis->itemID&&g_inst_section[0].name_ptr>=0))]);
...
static BOOL CALLBACK InstProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg == WM_INITDIALOG)
  {
...
#endif
    {
      ...
      {
#ifdef NSIS_CONFIG_COMPONENTPAGE
        if (g_inst_section[x].default_state&0x80000000)
#endif
