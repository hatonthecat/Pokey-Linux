...
// $Id: ie_impexp_MSWrite.cpp,v 1.2 2001/10/24 23:20:16 hub Exp $
...
int read_wri_struct_mem (struct wri_struct *cfg, unsigned char *blob) {
	...
	case CT_VALUE:
	    ...
	    break;
	case CT_BLOB:
	    cfg[i].data = (char*) malloc (cfg[i].size);
	    if (!cfg[i].data) {
...
int read_wri_struct (struct wri_struct *cfg, FILE *f) {
    int size, i;
    unsigned char *blob;
...
int wri_ole_read (unsigned char *data, int size, FILE *fout) {
#if 0
   FILE *f; 
